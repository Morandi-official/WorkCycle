# WorkCycle

一个适合部署到 Cloudflare Pages 的月度工作记录网站。

域名：`log.morandi.dpdns.org` 或 `worklog.morandi.dpdns.org`（长域名存在 BUG，暂停使用）

## 功能

- 以“月”为主体显示当月日历。
- 每周三显示“汇报日”标记。
- 每周四至下周三作为一个工作周期，用浅色背景分隔。
- 每一天都可以填写工作内容。
- 支持对选中的文字添加浅色荧光标注。
- 绑定 Cloudflare D1 后，记录会保存到云端；未绑定 D1 时，会自动保存到当前浏览器本地。

## Cloudflare Pages 部署参数

```text
Framework preset: None
Build command: 留空 或 exit 0
Build output directory: /
Root directory: 留空
```

## D1 绑定

在 Pages 项目的 Settings → Bindings 中添加 D1 database：

```text
Variable name: DB
Database name: workcycle-db
```

绑定后重新部署一次。

## 自定义域名

建议在 Pages → Custom domains 中添加：

```text
log.morandi.dpdns.org
```

如果 Cloudflare 没有自动创建 DNS，请添加：

```text
Type: CNAME
Name: log
Target: 你的 Pages 默认域名，例如 workcycle.pages.dev
Proxy status: Proxied
```

## 更新记录

### 2026-05-26

#### 周一开头月份的日历排版修复

- 修复当月 1 号正好是周一时，日历第一格直接显示本月 1 号，导致前面没有上月末日期、后面多出两周下月日期的问题。
- 新增 `calendarGridStartDate()`，当月份从周一开始时，日历从上一个周一开始显示，补齐上月最后一周。
- 保持日历固定显示 6 行 7 列，统一为“前一月尾部 + 当月 + 下一月开头”的结构。
- 已排查类似问题：从周二到周日开头的月份仍按原逻辑补齐上月尾部；从周一开头的月份走专门逻辑补齐上月整周。
- 更新 `index.html` 资源版本号到 `v=13`。

#### 日历格日期数字对齐修复

- 修复某个日期格填写内容后，日期数字向上偏移、与同行其他日期数字不对齐的问题。
- 将 `.day-cell` 改为纵向 flex 布局，固定顶部日期行高度。
- 固定 `.day-topline`、`.day-number`、`.report-badge` 的高度和行高，使日期数字不受下方预览内容影响。
- 调整 `.day-preview` 为弹性内容区，避免文字预览挤压顶部日期行。
- 更新 `index.html` 资源版本号到 `v=12`。

#### 荧光标注稳定性与卡顿优化

- 修复选中文字后点击颜色按钮时偶发无法上荧光色的问题。
- 增强选区保存机制：鼠标、键盘、触控和全局 `selectionchange` 都会缓存编辑器内的有效选区。
- 点击荧光按钮时优先使用当前实时选区，实时选区丢失时再使用缓存选区。
- 点击颜色按钮不会再抢走编辑器焦点，减少“时好时不好”的情况。
- 将日历格预览同步改为短延迟更新，自动保存延迟加长到 1100ms，减少输入和上色时的卡顿。
- 高亮失败时会在保存状态位置提示“请先选中文字”或“高亮失败，请重新选中文字”。
- 更新 `index.html` 资源版本号到 `v=11`。

#### GitHub 项目入口模块独立显示

- 将 `Dual-Motor-SMO-Control` 项目跳转入口从编辑卡片内部移出。
- 新增右侧 `side-panel` 结构，使 GitHub 项目入口成为编辑区上方独立白底模块。
- 编辑区仍保持独立白底卡片，荧光按钮和输入框不再与项目入口混在同一块中。
- 调整右侧布局间距、输入框高度与 sticky 行为。
- 更新 `index.html` 与 `styles.css` 资源版本号到 `v=10`。

#### GitHub 项目入口模块样式微调

- 进一步将 GitHub 跳转模块和编辑功能区拆分成两个独立白底卡片。
- GitHub 模块放在右侧栏最上方，荧光按钮、输入框、保存按钮放在下方独立编辑卡片中。
- 调整 `side-panel`、项目入口卡片、编辑框高度，避免右侧内容过高。
- 更新 `index.html` 和 `styles.css`，资源版本号更新到 `v=10`。

### 2026-05-25

#### 荧光标注即时同步修复

- 修复编辑框中有荧光标注但日历格预览偶尔不显示的问题。
- 编辑区内容变化时，立即同步当前日期的日历格预览。
- 增强高亮结构识别，兼容 `mark[data-color]`、带高亮 class 的 `mark`，以及部分带背景色样式的 `span`。
- 更新 `index.html` 资源版本号到 `v=8`。

#### 荧光标注选项与清除功能修复

- 在原有黄色、绿色、蓝色、粉色基础上，新增橙色、紫色、灰色 3 个荧光标注选项。
- 修复“清除”按钮无效的问题。
- 清除按钮现在支持两种操作：如果选中了文字，只清除选中文字中的荧光标注；如果没有选中文字，则清除当前编辑区里的全部荧光标注。
- 优化编辑器选区保存逻辑，避免点击荧光按钮时选区丢失。
- 同步更新日历格预览逻辑，新增颜色也能在日历格中显示。
- 更新 `index.html` 资源版本号，避免浏览器或 Cloudflare 继续加载旧脚本。

#### 更新记录维护方式调整

- 后续更新记录统一写在 `README.md` 的“更新记录”部分。
- 删除独立的 `CHANGELOG.md` 文件。

#### 日历格荧光标注显示修复

- 修复日历格预览只显示纯文字、不显示荧光标注的问题。
- 修改 `app.js`：日历格预览从纯文本渲染改为安全保留 `mark[data-color]` 标签。
- 修改 `styles.css`：为日历格中的黄色、绿色、蓝色、粉色荧光标注补充样式。
- 修改 `index.html`：更新资源版本号，避免浏览器或 Cloudflare 缓存旧脚本。

#### 空白日期显示优化

- 修改 `app.js`：没有填写内容的日期格子保持空白。
- 移除空白日期格中原先显示的“点击填写今日工作内容”。
- 修改 `index.html`：更新资源版本号，避免继续加载旧脚本。

#### 页面布局压缩优化

- 修改 `index.html`：将页面主体改为两栏布局。
- 修改 `styles.css`：压缩顶部标题、月份栏、图例和日历格高度。
- 将编辑区放到右侧，减少页面垂直滚动。
- 右侧编辑区使用 sticky 布局，方便在查看日历时保持编辑入口可见。
- 缩短荧光按钮文案为“黄 / 绿 / 蓝 / 粉 / 清除”。

#### 初版功能构建

- 新建 WorkCycle 月度工作记录网站。
- 新增 `index.html`：页面结构，包括标题、月份切换、图例、月历、每日记录编辑区。
- 新增 `styles.css`：浅色风格、日历格、周期底色、汇报日标记、编辑器和荧光按钮样式。
- 新增 `app.js`：月份切换、日期选择、每日记录编辑、自动保存、荧光标注、本地存储回退。
- 新增 `functions/api/records.js`：Cloudflare Pages Functions API，用于通过 D1 保存和读取每日工作记录。
- 新增 `README.md`：部署说明、D1 绑定说明、推荐域名说明。
