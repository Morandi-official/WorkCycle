...
- 将 `Dual-Motor-SMO-Control` 项目跳转入口从编辑卡片内部移出。
- 新增右侧 `side-panel` 结构，使 GitHub 项目入口成为编辑区上方独立白底模块。
- 编辑区仍保持独立白底卡片，荧光按钮和输入框不再与项目入口混在同一块中。
- 调整右侧布局间距、输入框高度与 sticky 行为。
- 更新 `index.html` 与 `styles.css` 资源版本号到 `v=10`。

#### GitHub 项目入口模块样式微调

- 进一步将 GitHub 跳转模块和编辑功能区拆分成两个独立白底卡片。
- GitHub 模块放在右侧栏最上方，荧光按钮、输入框、保存按钮放在下方独立编辑卡片中。
- 调整 `side-panel`、项目入口卡片、编辑框高度，避免右侧内容过高。
- 更新 `index.html` 和 `styles.css`，资源版本号更新到 `v=10`。

### 2026-05-25

#### 荧光标注即时同步修复

- 修复编辑框中有荧光标注但日历格预览偶尔不显示的问题。
- 编辑区内容变化时，立即同步当前日期的日历格预览。
- 增强高亮结构识别，兼容 `mark[data-color]`、带高亮 class 的 `mark`，以及部分带背景色样式的 `span`。
- 更新 `index.html` 资源版本号到 `v=8`。
